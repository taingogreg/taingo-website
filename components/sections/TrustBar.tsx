  ...
  return (
    <section className="bg-bone border-y border-bone py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <p className="text-center text-sm font-extrabold uppercase tracking-[0.22em] text-midnight mb-8">
        ...
        <div className="flex flex-wrap justify-center gap-6 md:gap-10">
          {industries.map(({ name, icon: Icon }) => (
            <div key={name} className="flex flex-col items-center gap-2 group">
              ...
              <span className="text-xs font-semibold text-midnight">{name}</span>
            </div>
          ))}
        </div>
      </div>
